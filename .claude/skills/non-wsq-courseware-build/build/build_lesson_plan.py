...
def _load_domains():
    ...
    for f in sorted(_glob.glob(os.path.join(HERE,"data_domain[0-9]*.py")),
        ...
        # glob and would otherwise double every activity in the schedule.
...
for day,(theme,rows) in SCHEDULE.items():
    ...
    r=doc.add_paragraph().add_run(
        f"Total instructional time: {inst} minutes ({inst/60:g} hours), 9:30 am - 5:30 pm."
        + (f" Plus {tea} minutes of tea breaks." if tea else ""))
    r.italic=True; r.font.size=Pt(9.5); r.font.color.rgb=GREY
    # The non-WSQ house day is 9:30am-5:30pm = 450 instructional minutes
    # (7.5 hours) plus a single 30-minute lunch, NOT the WSQ 480.
    assert inst==450, f"Day {day} instructional minutes = {inst}, expected 450"

H("Activity Reference (aligned to course topics)",1)
