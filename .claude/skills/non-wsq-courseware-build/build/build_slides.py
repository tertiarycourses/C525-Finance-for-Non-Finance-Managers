...
def _load_domains():
    ...
    for f in files:
        # Only exact data_domainN.py files. Editor/Drive duplicates such as
        # "data_domain1 2.py" match the glob and would double every activity.
...
def head(s,title,kicker=None,kcolor=BLUE):
    ...
    # Auto-fit the title so long titles never wrap into the divider rule below.
    # The ladder runs down to 13pt: a title long enough to need two lines at 17pt
    # (a day theme reused as a heading, say) would otherwise overlap the rule.
    L=len(str(title))
    tsize = 29 if L<=40 else (21 if L<=52 else (17 if L<=72 else (15 if L<=95 else 13)))
...
def cards3(title,cards,kicker):
    ...
    # the row instead of huddling at the left edge.
...
def tile_grid(title,items,kicker=None,cols=2,size=15,icons=None,accent=BLUE,_start=0,_fixed_rows=None):
    ...
    if len(items)>TILE_MAX:
        last=None
        # Spread items evenly across the pages rather than filling each page to
        # TILE_MAX: 7 items become 4+3, not a full page followed by a lone tile.
    ...
    for i,it in enumerate(items):
        ...
        r=i//cols; c=i%cols
        x=int(X0+(cw+gx)*c)
        # Centre a lone tile on the final row instead of stranding it left.
...
def shot(title,img,kicker=None,caption=""):
    """Screenshot slide — a framed, undistorted capture with a caption.

    The image is scaled to fit the content area at its OWN aspect ratio; a
    portrait page or a non-16:10 capture is letterboxed rather than stretched."""
    ...
    if caption:
        # A long caption gets two lines at a smaller size instead of clipping.
        long_caption=len(caption)>120
        txt(s,Inches(0.85),Inches(6.43 if long_caption else 6.68),Inches(11.6),
            Inches(0.54 if long_caption else 0.35),
...
# Initials are derived from the name so the badge can never disagree with the
# name printed beneath it when the course is delivered by a different trainer.
...
tile_grid("Ground Rules",[
 ...
 "Be punctual; return from breaks on time.","Learn by doing — try every hands-on activity."],
...
_dl=flow_h("Download Course Material",[
 ...
 "Keep them open alongside the activities as you work"],kicker="COURSE PORTAL")
...
def _lab_range(acts):
    """Render an activity list compactly: contiguous runs collapse to
    'Activities 2-5', gaps are listed, and a single one reads 'Activity 1'."""
    ...
    return f" ({'Activity' if len(ns)==1 else 'Activities'} {', '.join(parts)})"
def _topic_line(t):
...
if C.DAYS == 1:
    # A one-day course has no "Day 2" to put in the right column (and indexing
...
flow_h("How You'll Learn",[
 ...
 "You build it yourself in the hands-on activity",
 "Verify with the activity's 'Test it' step",
...
def _lab_extras(num):
    for fn,label,cap in _SHOTS.get(num,[]):
        ...
        if os.path.exists(p):
            shot(f"{label}",p,kicker=f"ACTIVITY {num} · SCREENSHOT",caption=cap)
...
for t in C.TOPICS:
    ...
    # A one-card row is stretched whitespace, and a no-card row is an empty slide;
    # the per-activity overview that follows already says everything either would.
    if len(acts) > 1:
        cards3(f"Hands-On Activities — {t['title']}", cards, kicker="WHAT YOU'LL DO")
    # per activity
    for a in acts:
        activity_overview(f"ACTIVITY {a['num']}", a["title"], a["desc"], a["build"], a["services"], kicker=f"TOPIC {t['code']} · HANDS-ON")
        _lab_extras(a["num"])
        ...
        for i,(instr,cmd) in enumerate(steps,1):
            step_slide(f"ACTIVITY {a['num']}", a["title"], i, total, instr, cmd)
        test_slide(a["title"], a["test"], kicker=f"ACTIVITY {a['num']} · VERIFY")
    # topic recap. A topic with no activity has no objectives to recap, which
...
content("Keep Practising After the Course",[
 "Re-run the activities on your own machine — repetition is what makes the skills stick.",
...
big_statement("Thank You!",
              _ty.get("body","You now have the practical skills from every activity in this course — keep applying them to your own work."),
